  ...
  public function alterBuild(array &$build, EntityInterface $entity, EntityViewDisplayInterface $display, $view_mode) {
    ...
    if ($display->getComponent('score') && $entity->canReview('score')) {
      $params = [
        ...
        '@username' => ($account->id() == \Drupal::currentUser()->id()) ? $this->t('You') : $account->getDisplayName(),
        '@score' => $score['percentage_score'],
        '@yourtotal' => ($account->id() == \Drupal::currentUser()->id()) ? $this->t('Your') : $this->t('Total'),
      ];

      // Show score.
      $build['score']['#markup'] = '<div id="quiz_score_possible">' . $this->t('@username got %num_correct of %question_count possible points.', $params) . '</div>' . "\n";
      $build['score']['#markup'] .= '<div id="quiz_score_percent">' . $this->t('@yourtotal score: @score%', $params) . '</div>';
    }
